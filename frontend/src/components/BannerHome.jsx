...
const BannerHome = ({  onSearch }) => {
    ...
    const handleSubmit = (e) => {
        ...
        if(trimmedTerm) {
            if(onSearch) {
                const searchWords = trimmedTerm.toLocaleLowerCase().split(/\s+/)
  ...
  return (
    <div className=' relative overflow-hidden pt-16'>
     ...
     <div className=' relative z-10 mt-8 sm:mt-10 lg:mt-12 max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-8 sm:py-12 md:py-16'>
        <div className=' grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-16 items-center md:items-start'>
            {/* LEFT CONTENT */}

            <div className=' text-center md:text-left'>
                <div className={bannerStyles.tag}>
                    <span className=' flex items-center text-sm sm:text-base'>
                        <FiTruck className=' mr-2' />Free delivery on orders over Rs 500
                    </span>
                </div>

                {/* <h1 className={bannerStyles.tag}>
                    Fresh{' '}
                    ...
                    <h1 >
                    <span className="text-black text-2xl sm:text-3xl font-semibold">
                        Fresh
                        </span>{' '}
                        <span className="text-emerald-600 text-4xl sm:text-5xl italic font-bold">
                            Groceries
                            </span>{' '}
                    ...
                    <p className={bannerStyles.paragraph}>
                     ...
                     <form onSubmit={handleSubmit} className={bannerStyles.form}>
                        <input type="text" values={searchTerm} onChange={handleSearch} placeholder='Search for fruits, vegetables, meats, dairy..' className={bannerStyles.input}/>
                        ...
                        <div className=' grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4'>
                            {features.map((f,i) =>(
                                <div key={i} className={bannerStyles.featureItem}>
                                    <div className=' text-teal-600 mb-1'>{f.icon}
                            ...
                            {/* RIGHT IMAGE */}
                            <div className=' relative flex justify-center md:pl-12 lg:pl-20'>

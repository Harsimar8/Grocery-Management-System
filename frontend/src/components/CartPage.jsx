            ...
            <div className={contactStyles.pageContainer}>
                <div className={cartStyles.maxContainer}>
                   ...
                   <h1 className={cartStyles.emptyCartHeading}>Your Cart is Empty</h1>
                     <p className={cartStyles.emptyCartText}>Looks like you haven't added ant organic goodies to your cart yet.</p>
                    ...
                    <div className={cartStyles.cartItemsGrid}>
                        {cart.map((item) => (
                            <div key={item.id} className={cartStyles.cartItemCard}>
                                ...
                                <h3 className={cartStyles.cartItemName}>
                                    {item.name}
                                    </h3>
                                <p className={cartStyles.cartItemPrice}>Rs{(item.price ?? 0).toFixed(2)}</p>
                                {/* ADD CONTROLS */}

                                <div className={cartStyles.cartItemQuantityContainer}>
                                    <button onClick={() => handleQuantityChange(item.id, -1)} className={cartStyles.cartItemQuantityButton}><FiMinus/></button>

                                    <span className={cartStyles.cartItemQuantity}>{item.quantity}</span>
                                    <button onClick={() => handleQuantityChange(item.id, 1)} className={cartStyles.cartItemQuantityButton}><FiPlus/></button>
                                    </div>
                ...
                <div className={cartStyles.orderSummaryCard}>
                    ...
                    <div className=' space-y-4 text-sm sm:text-base'>
                        <div className={cartStyles.orderSummaryRow}>
                            ...
                            <span className={cartStyles.orderSummaryValue}>Rs{getCartTotal().toFixed(2)}</span>
                            </div>
                            ...
                            <span className={cartStyles.orderSummaryValue}>Rs{(getCartTotal() * 0.05).toFixed(2)}</span>
                            </div>
                            ...
                            <div className={cartStyles.orderSummaryTotalRow}>
                                ...
                                <span className={cartStyles.orderSummaryTotalValue}>Rs{(getCartTotal() * 1.05).toFixed(2)}</span>
                            </div>
                    </div>
